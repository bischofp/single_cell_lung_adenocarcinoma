...
count_down <- count_raw

for (i in seq_along(1:ncol(count_down))){
  repeat{
    x <- sample(1:nrow(count_down),1)
    count_down[x,i] <- ifelse(count_down[x,i] > 0, count_down[x,i]-1, count_down[x,i])
    if(colSums(count_down)[[i]] == 1000){
      break
    }
  print(paste0("i=",i))
  }
